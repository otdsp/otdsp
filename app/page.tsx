...
// -----------------------------------------------------------------------------
function SolucaoBlock({ num, title, description, category, categoryColor, theme, align = 'left', className = '', Icon }: any) {
  return (
    <div className={`case-block relative w-full flex items-center justify-center md:justify-${align === 'left' ? 'start' : 'end'} h-auto min-h-[400px] md:min-h-[450px] py-8 md:py-0 ${className}`}>
      
      {/* Background Wrapper */}
      <div className="absolute inset-0 rounded-3xl overflow-hidden shadow-2xl grid grid-cols-1 md:grid-cols-2">
        {align === 'left' ? (
          <>
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 h-full w-full relative overflow-hidden">
               <div className="absolute -left-20 -bottom-20 w-80 h-80 bg-cyan-500 rounded-full blur-[100px] opacity-[0.2]"></div>
               <h2 className="absolute text-[6rem] md:text-[8rem] font-bold tracking-tighter uppercase opacity-[0.03] text-white pointer-events-none z-0 top-1/2 -translate-y-1/2 left-4 md:left-10">
                SOL 0{num}
               </h2>
            </div>
            <div className="relative w-full h-full hidden md:block">
              <Image 
                src={`https://picsum.photos/seed/sol${num}/800/600`} 
                alt={title} 
                fill 
          ...
          <>
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 h-full w-full relative overflow-hidden">
               <div className="absolute -right-20 -top-20 w-80 h-80 bg-orange-500 rounded-full blur-[100px] opacity-[0.2]"></div>
               <h2 className="absolute text-[6rem] md:text-[8rem] font-bold tracking-tighter uppercase opacity-[0.03] text-white pointer-events-none z-0 top-1/2 -translate-y-1/2 right-4 md:right-10">
                SOL 0{num}
               </h2>
            </div>
          </>
      ...
      {/* Floating Text Card */}
      <div className={`relative z-10 bg-white rounded-2xl shadow-xl p-6 md:p-8 max-w-xl mx-4 md:mx-0 ${align === 'left' ? 'md:-ml-8' : 'md:-mr-8'}`}>
...
export default function Page() {
  ...
  return (
    ...
    >
      ...
      <section className="relative w-full py-10 flex flex-col items-center gap-24 md:gap-32 max-w-7xl mx-auto px-4">
           {/* CASE 1 */}
        ...
        <div className="case-block relative w-full h-auto p-6 md:p-16 lg:p-20 shadow-2xl shadow-slate-900/5 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center rounded-[2.5rem] bg-white/70 backdrop-blur-xl border border-white">
          ...
          <div className="relative w-full h-full min-h-[400px] lg:min-h-[500px]">
            <Image 
              src="https://picsum.photos/seed/case2/800/800" 
              alt="Projeto INSPIRE" 
              fill 
              className="object-cover rounded-2xl shadow-lg"
              referrerPolicy="no-referrer"
      ...
      <section className="relative w-full max-w-7xl mx-auto py-32 px-4 md:px-0">
        <div className="text-center mb-24 px-4">
          ...
          <p className="text-base md:text-lg text-slate-500 font-medium max-w-2xl mx-auto">Diretrizes inovadoras prontas para a implementação municipal.</p>
        </div>

        <div className="flex flex-col space-y-32 md:space-y-40 w-full px-4 md:px-10 pb-10">
          {/* Solução 1: Left Aligned */}
          <div className="w-full md:w-11/12 xl:w-4/5 mr-auto relative z-10 flex items-center">
            <SolucaoBlock 
              num={1}
              title="Soluções Estratégicas em Saúde" 
          ...
          <div className="w-full md:w-11/12 xl:w-4/5 ml-auto relative z-20 flex items-center">
            <SolucaoBlock 
              num={2}
          ...
          <div className="w-full md:w-11/12 xl:w-4/5 mr-auto relative z-30 flex items-center">
            <SolucaoBlock 
              num={3}
          ...
          <div className="w-full md:w-11/12 xl:w-4/5 ml-auto relative z-40 flex items-center">
            <SolucaoBlock 
              num={4}
